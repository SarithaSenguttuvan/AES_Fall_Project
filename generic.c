...
uint8_t send_hb_req(msgStruct_t * HB_req)
{
...
uint8_t send_heartBeat(int8_t srcTaskId, msgStruct_t *HB_main)
{
	...
	if((qdes_hb = mq_open(MAIN_TASK_MQ_NAME, O_NONBLOCK | O_WRONLY)) == (mqd_t)-1) 
	{
    	printf ("send_heartBeat::ERROR No: %d Main queue unable to open\n", errno);
    }
    printf("send_heartBeat::Main message queue opened\n");
    
	if(mq_send(qdes_hb, (char *)HB_main, sizeof(msgStruct_t), 0) == -1) 
    {
...
uint8_t send_log(uint8_t srcTaskId, log_t* logPacket)
{
	mqd_t msg_qdes;
...
void blockSignals(int taskID)
{
	...
	if (rc != 0)
   		printf("main::Sigmask Unblock error\n");
...
int32_t unblockOnSignal(int taskID)
{
	...
	/*rc = pthread_sigmask(SIG_UNBLOCK, &globalMask_set, NULL);
	if (rc != 0)
   		printf("main::Sigmask Unblock error\n");*/
